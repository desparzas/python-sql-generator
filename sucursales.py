...
def main():
    ...
    df = pd.read_excel('./data/1442 - (240117).AR - Cargar Sucursales.xlsx', sheet_name='Hoja de Trabajo')
    ...
    for _, row in df.iterrows():
        ...
        insert_query = f"INSERT INTO despegar.sucursal({columns_str}) VALUES ({values_str});"
        insert_queries.append(insert_query)

    
    
    # guardar las consultas en un archivo, encoding UTF-8
    with open('./scripts_sql/insert_sucursales_argentina.sql', 'w', encoding='utf-8') as f:
        f.write('\n'.join(insert_queries))
